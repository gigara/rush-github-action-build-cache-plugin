...
import { IRushPlugin, RushConfiguration, RushSession } from '@microsoft/rush-lib';
...
export default class RushCustomBuildCachePlugin implements IRushPlugin {
    ...
    public apply(rushSession: RushSession, rushConfiguration: RushConfiguration) {
        rushSession.hooks.initialize.tap('rush-github-action-build-cache-plugin', () => {
            rushSession.registerCloudBuildCacheProviderFactory(
                'github-action-build-cache',
                (buildCacheConfig): GithubCacheProvider => {                    
                    return new GithubCacheProvider(rushConfiguration);
                }
            );
        });
    }
